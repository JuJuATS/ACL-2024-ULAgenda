...
userForm.addEventListener("submit",(e)=>{
  ...
  fetch("/signin",fetchOptions).then(res=>res.json()).then(data=>data.success ? window.location.href = "/" : afficherPopUp(data.message,false))
})
...
function afficherPopUp(text, good) {
  ...
  popUp.innerHTML = good ? `
      <svg class="error" viewBox="0 0 512 512">
  ...
  ` :
      `
      <svg class = "succes" viewBox="0 0 48 48">
